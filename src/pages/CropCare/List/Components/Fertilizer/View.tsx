...
const Fertilizer: FC<Props> = (props) => {
    const { landData, fertilizerOptions, formState, onAddNewFertilizer, onRemoveFertilizer, onChangeFormState, onPrevious, onSubmit } = useViewModel(props);
    ...
    return (
        <Fragment>
            <div className="grid grid-cols-12 gap-5">
                ...
                <div className="col-span-12">
                    <button type="button" className="btn bg-crop-primary text-white font-bold px-4 py-2 hover:opacity-80 cursor-pointer w-full shadow border-none" onClick={onAddNewFertilizer}>
                ...
                {(formState?.fertilizer ?? []).map((_: any, index: number) => (
                    <div className="col-span-12">
                        <RoundItem key={index} index={index} formState={formState} onChange={onChangeFormState} onRemove={onRemoveFertilizer} options={fertilizerOptions} />
                    </div>
                ))}
