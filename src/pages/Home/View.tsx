                    ...
                    {filteredBooks.length === 0 ? (
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 sm:gap-6 mx-auto max-w-5xl">
                            {filteredBooks.map((book) => (
                                <div key={book.id} className="flex flex-col items-center p-3 bg rounded-lg sm:h-60" onClick={() => navigateTo(book.id)}><img
                                    loading="lazy"
                                    src={`https://deerandbook.com/${book.cover_image}`}
                                    alt={book.book_category_name}
                                    className="mt-2 w-full h-40 rounded-md object-contain"
                                />
